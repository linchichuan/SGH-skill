...
7. If the quote is missing or expired, `execution_eligible` is not exactly `true`, or `funding_status` is not an approved paid/sponsored value, stop. Tell the user that no call or human task was created and route them to the official SGH quote or contract channel. Missing commercial fields fail closed.
...
9. Only after the commercial gate and explicit confirmation, call `confirm_assistance_request` with the draft's exact `contract_version`, `explicit_confirmation=true`, and a stable `idempotency_key`. Reuse the same version and key only when retrying that exact confirmation.
